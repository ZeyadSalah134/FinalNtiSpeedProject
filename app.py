...
def inject_css():
    st.markdown(
        ...
        /* Real card containers (st.container(border=True)) styled to match the
           REVORA design language. Replaces the old broken pattern of opening
           a <div class="ap-card"> in one st.markdown() call and closing it in
           a later, separate call -- Streamlit renders each markdown call as
           its own isolated HTML fragment, so that pattern produced empty,
           orphaned boxes instead of an actual wrapping card. */
        div[data-testid="stVerticalBlockBorderWrapper"]:has(> div > div[data-testid="stVerticalBlock"]) {{
            background: {COLORS['surface']};
            border: 1px solid {COLORS['surface_border']} !important;
            border-radius: 16px;
            margin-bottom: 0.9rem;
        }}
    ...
    st.markdown(
        ...
        <div class="ap-hero">
            ...
            <div class="revora-status">
                ...
                <span class="status-sep">|</span> MODEL YEAR RANGE: <b>1950&ndash;2026</b>
            </div>
        </div>
        """),
        unsafe_allow_html=True,
